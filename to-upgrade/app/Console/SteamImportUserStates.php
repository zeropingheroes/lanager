...
use Zeropingheroes\Lanager\Domain\States\State;
use Zeropingheroes\Lanager\Domain\Applications\Application;
use Zeropingheroes\Lanager\Domain\Servers\Server;
...
class SteamImportUserStates extends BaseCommand
{
    ...
    public function fire()
    {
        ...
        if (count($missingApps) > 0) {
            $this->error(count($missingApps).' '.str_plural('application',
                    count($missingApps)).' missing from local database - Please run "steam:import-apps"');
        }
    }
