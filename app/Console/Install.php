...
class Install extends BaseCommand
{
    ...
    public function fire()
    {
        if (Config::get('lanager/config.installed')) {
            if (!$this->confirm('Installation already marked as completed - are you sure you want to continue? [yes|no]')) {
                exit();
            }
        }
    ...
    private function runInstaller()
    {
        ...
        if ($migrate != 0) {
            $this->abort('Database structure creation failure', $this->criticalMessage);
        }
        ...
        if (!$sessionConfig) {
            $this->abort('Unable to change session driver', $this->criticalMessage);
        }

        $this->info('Setting application key...');
        $appKey = Artisan::call('key:generate');
        if ($appKey != 0) {
            $this->abort('Unable to set application key', $this->criticalMessage);
        }
