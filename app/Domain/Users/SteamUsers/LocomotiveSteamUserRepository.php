...
class LocomotiveSteamUserRepository implements SteamUserContract
{
    ...
    public function getUser($steamId64)
    {
        if (strlen($steamId64) != 17) {
            throw new \InvalidArgumentException('SteamId64 must be 17 characters in length');
        }

        $steamUsers = $this->getUsers([$steamId64]);
    ...
    public function getUsers(array $steamId64s)
    {
        ...
        foreach ($profiles->response->players as $profile) {
            ...
            if (isset($profile->timecreated)) {
                $steamUser->creation_time = $profile->timecreated;
            }
            if (isset($profile->avatar)) {
                $steamUser->avatar_url = $profile->avatar;
            }
            if (isset($profile->primaryclanid)) {
                $steamUser->primary_group_id = $profile->primaryclanid;
            }
            if (isset($profile->personastate)) {
                $steamUser->status = $profile->personastate;
            }
            if (isset($profile->lastlogoff)) {
                $steamUser->last_online_time = $profile->lastlogoff;
            }
            if (isset($profile->gameextrainfo)) {
                $steamUser->current_app_name = $profile->gameextrainfo;
            }
            ...
            if (isset($profile->loccityid)) {
                $steamUser->location_city_id = $profile->loccityid;
            }
            if (isset($profile->loccountrycode)) {
                $steamUser->location_country_code = $profile->loccountrycode;
            }
            if (isset($profile->locstatecode)) {
                $steamUser->location_state_code = $profile->locstatecode;
            }
            if (isset($profile->gameid)) {
                $steamUser->current_app_id = $profile->gameid;
            }
